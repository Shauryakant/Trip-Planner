...
2. Every stop MUST have a unique UUID string for "id".
3. Ensure times are formatted consistently (e.g., 09:00 AM, 01:30 PM, Evening).
4. Output ONLY the raw JSON object.
5. If the user's description does not contain enough information to build a real trip itinerary (e.g. it's empty, gibberish, unrelated to travel, or has no identifiable destination or duration), respond with exactly:
{ "trip_title": "", "days": [] }
Do not guess a destination or fabricate a generic trip if the input is insufficient.`;
