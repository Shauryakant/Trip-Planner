 ...
 * @returns {{ valid: boolean, reason?: "empty" | "malformed" | "wrong_shape" | "insufficient_input", error?: string, data?: object }}
 */
export function validate(data) {
  ...
  // Early check for intentionally empty model signal (insufficient input)
  const isDaysEmpty = !Array.isArray(data.days) || data.days.length === 0;

  if (isDaysEmpty) {
    return {
      valid: false,
      reason: 'insufficient_input',
      error: 'Please describe your trip with a bit more detail — a destination and how many days works well.',
    };
  }
